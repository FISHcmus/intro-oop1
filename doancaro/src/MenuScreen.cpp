...
static Texture2D s_menuBg     = {};
static bool      s_menuBgTried = false;

static void ensureMenuBg() {
    if (s_menuBgTried) return;
    s_menuBgTried = true;
    s_menuBg = LoadTexture("assets/images/menu-background-1.png");
    if (s_menuBg.id != 0) {
        SetTextureFilter(s_menuBg, TEXTURE_FILTER_BILINEAR);
    }
}
...
static void drawMenuBackground(int w, int h) {
    ...
    const auto iw = static_cast<float>(s_menuBg.width);
    const auto ih = static_cast<float>(s_menuBg.height);
    const float scale = (wf / iw) > (hf / ih) ? (wf / iw) : (hf / ih);
    const float dw = iw * scale;
    const float dh = ih * scale;
    Rectangle src = { 0.0f, 0.0f, iw, ih };
    Rectangle dst = { (wf - dw) * 0.5f, (hf - dh) * 0.5f, dw, dh };
    DrawTexturePro(s_menuBg, src, dst, {0, 0}, 0.0f, WHITE);
}
